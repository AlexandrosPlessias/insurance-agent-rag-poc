...
export function ChatPage({
  ...
  // Only fires when history is empty (post-RESET), so it never
  // overwrites an in-progress streaming session.
  useEffect(() => {
  ...
  const handleSubmit = async (text: string) => {
    ...
    try {
      await processStream(
        streamChat({ question: text, user_id: userId, conversation_id: conversationId })
      );
    } catch (e) {
  ...
  const handleApproved = async () => {
    ...
    dispatch({ type: "PUSH_ASSISTANT_TURN" });
    try {
      await processStream(streamPlanResume(planId));
    } catch (e) {
      dispatch({ type: "SET_STREAM_ERROR", error: String(e) });
    }
  };
        ...
        {isEmpty ? (
          ...
          <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
            {state.history.map((turn, i) => {
              // Skip empty closed assistant turns (pre-execution gate: no content, not streaming)
              if (turn.role === "assistant" && !turn.isStreaming && !turn.content.trim() && Object.keys(turn.stages).length === 0) return null;
              return (
                <ChatMessage
                  key={i}
                  turn={turn}
