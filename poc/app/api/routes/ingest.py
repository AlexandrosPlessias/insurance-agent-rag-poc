...
Wraps `app.ingestion.pipeline.ingest_document` so the same per-document
flow used by `scripts/ingest_pdfs.py` is available to the Streamlit
upload form (and any future UI).
...
async def ingest_pdf(
    ...
    # Persist the upload under data/knowledge_base/raw/ so the URL-based
    # /sources/<filename> download keeps working unchanged.
    settings.raw_pdf_dir.mkdir(parents=True, exist_ok=True)
    target = settings.raw_pdf_dir / file.filename
