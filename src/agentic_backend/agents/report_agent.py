"""Report agent — Markdown policy summary and executive annual report."""
import json
import re
import time
...
from agentic_backend.agents.memory_agent import format_user_activity
...
from agentic_backend.reporting.charts import render_premium_chart
from agentic_backend.reporting.markdown import build_policy_report
...
def _executive_report(state: GraphState, target_year: int) -> dict:
    ...
    """
    from agentic_backend.reporting.executive import build_executive_report
    from agentic_backend.reporting.writers import render_markdown
    ...
    return {
        ...
        "validated": True,
        "retry_count": 0,
        # expose the run id + the doc payload so the UI
...
def report_node(state: GraphState) -> dict:
    ...
    when target_year is present, dispatches to the
    executive annual-report pipeline. Without a year we keep the
    single-policy Markdown summary.
    """
