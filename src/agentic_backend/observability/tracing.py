...
def annotate_request_span(
    ...
    *,
    ...
    """Tag a span with user.id, conversation.id, and plan.id when known.

    Also appends a short suffix to the span name (e.g. "[conv=42]") so
    the IDs are visible directly in the Aspire Traces list without having
    to expand the span. Safe to call with None values — missing fields are
    silently skipped. The name update is idempotent: re-calling with the
    same IDs is a no-op.

    Use from route handlers (root HTTP span) and from each graph node so
    the IDs are visible everywhere in Aspire.
    """
    if span is None:
        return
    if user_id:
    ...
    if conversation_id:
        try:
            ...
            pass
    if plan_id:
        span.set_attribute("plan.id", str(plan_id))
    # Append IDs to the span name so they appear in the Aspire trace list.
    suffix_parts = []
    if conversation_id:
        suffix_parts.append(f"conv={conversation_id}")
    if plan_id:
        suffix_parts.append(f"plan={str(plan_id)[:8]}")
    if suffix_parts:
        try:
            current_name: str = span.name
            if "[" not in current_name:
                span.update_name(f"{current_name} [{', '.join(suffix_parts)}]")
