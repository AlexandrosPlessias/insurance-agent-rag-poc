"""Auto-discovered Skill registry for the agentic architecture.
...
class Skill:
    ...
    requires_approval: bool = False              # pre-execution gate
...
def get_skill_registry() -> dict[str, Skill]:
    ...
    for _finder, module_name, _is_pkg in pkgutil.iter_modules([str(pkg_path)]):
        if module_name.startswith("_"):
            continue
        module = importlib.import_module(f"agentic_backend.skills.{module_name}")
