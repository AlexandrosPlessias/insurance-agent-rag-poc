...
import { approvePlan, rejectPlan, telegramPollStart, telegramPollStop } from "../api/client";
...
export function ApprovalCard({ event, onApproved, onRejected }: Props) {
  ...
  // Guard: onApproved/onRejected must fire at most once — prevents double
  // streamPlanResume when Telegram poll auto-detects AND user clicks button.
  const resolvedRef = useRef(false);

  const countdown = useCountdown(event.expires_at);
  const status = usePlanPolling(event.plan_id, 5000);

  // Auto-detect resolution via poll — in effect, never during render.
  useEffect(() => {
    if (resolvedRef.current) return;
    if (status?.state === "approved") { resolvedRef.current = true; onApproved(); }
    else if (status?.state === "rejected") { resolvedRef.current = true; onRejected(); }
  }, [status?.state]); // eslint-disable-line react-hooks/exhaustive-deps

  // Signal the Telegram bot to start polling on mount, stop on unmount.
  useEffect(() => {
    void telegramPollStart(event.plan_id);
    return () => { void telegramPollStop(event.plan_id); };
  }, [event.plan_id]);
  ...
  const handleApprove = async () => {
    ...
    catch (e) { resolvedRef.current = false; setError(String(e)); }
          ...
          <>
            <div style={{
              ...
              <div style={{ fontSize: 11, color: "#b45309", marginTop: 6 }}>
                ⏱ Listening for Telegram command…
              </div>
            </div>
